...
class Likes extends Component {
    constructor(props) {
        ...
        this.state = {
            liked: '', 
            postID: this.props.postID, sameUser: '', comment: '', numComments: '', comments: [], numLikes: 0
          }
    }

    componentDidMount () {
        this.setState({liked: ''});
        firebase.database().ref('posts/' + this.props.postID + '/likes/' + this.props.uid).once("value").then((snapshot)=> {
            if(snapshot.val() === true) {
                console.log("is liked", snapshot.val())
            }
 ...
 render() {
   return (
       <div>
    {this.state.liked ?       <div id="box indivpost" className="Post">
